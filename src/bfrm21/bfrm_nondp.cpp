...
void Bfrm::LoadData(Model& model) {
	...
	//BfrmData::SaveData("mX.txt",mXMask);
...
void Bfrm::Run(int nStartIter, Model& model) {
	...
	for (int it=1 + nStartIter; it <= model.mnMC + model.mnBurnin + nExtra; it++) {
		...
		Iterate(model.mnNLatentFactors, model.mnNDesignControls, model.mnPriorPiStandard, 0, 
			model.mdDPAlpha, model.mnNonGaussianFactors, model.mnInverseWishart);
...
void Bfrm::Initialise(Model& model, int FindFounder)
{
	...
	for (i = design + mnYFactors; i < nTotalFactor; i++) {
		for (j = 0; j < mnSampleSize; j++) {
			mIter.mF[i][j] = mWeight[j] ? mX[i - design][j] : 0;
		}
	}
	
	
	//BfrmData::LoadData("mA_fixed.txt", mIter.mB,mnVariables, nTotalFactor);
	//BfrmData::LoadData("mPsi_fixed.txt", mIter.mPsi,  mnVariables);
...
void Bfrm::RankVariables(int nSkip1, int nSkip2, int VarInCount) {
	int i;
...
void Bfrm::SaveIter(int i) {
	...
	BfrmData::SaveData("Tau.txt", mIter.maTau);
...
void Bfrm::Iterate(int factor, int design, int joel, int mode, double alpha, int nongaussian, int inversewishart) {

	ManageHelperVariables(true, factor);			//usefull when COM mode is used
	if (!mode) {
		if (nongaussian) {
			SampleF(factor, design, &mDP, inversewishart);
	...
	SampleB(factor, design, joel, mode, inversewishart);
	//mResX = mX - matmult(mIter.mB,mIter.mF);
...
void Bfrm::SampleB(int factor, int design, int joel, int mode, int inversewishart) {
	int i,j,k;
	int ntotalBz = 0;
		...
		if (v) {	
			...
			for ( i = 0; i < mnVariables; i++) {
				...
				SampleBij(i,j,v,design, oddratio, newvij, mPrior.GetPriorMean(i),postpi,0, 
					joel, j == nTotalFactor - 1, mode, inversewishart);
			...
			if (m == 0) {   
				...
				SampleBij(minindex,j,v,design, oddratio, newvij, 
					mPrior.GetPriorMean(minindex),postpi,1, joel, j == nTotalFactor - 1, mode, inversewishart);
				m++;
				if (joel) { 
...
void Bfrm::SampleBij(int i, int j, double v, int design, double oddratio, double& newv, double meanvalue, 
	double &postpi, int specialcase, int joel, int lastfactor, int mode, int inversewishart) {
...
//update mF mResX
void Bfrm::SampleF(int factor, int design, mdp* pdp, int inversewishart) {
	int i,j,k,m;
	int N = factor + mnYFactors;
	if (N <= 0) return;
...
//----------------------------------------------------------------- Wishart Only
	...
	if (inversewishart) {
		...
		try {
			...
			IW.Init(mPrior.mInverseWishartT0,pdp->mnGroups, N,  mPrior.mT,ThetaSquare);
			IW.Next(mIter.mT);
		} catch (...) {
